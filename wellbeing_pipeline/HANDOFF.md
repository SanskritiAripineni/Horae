...
The `Behavioral Emotional Sensing` project was doing binary momentary-mood classification on StudentLife + K-EmoPhone with LOSO evaluation and hitting a 55–64% balanced-accuracy ceiling. That ceiling is a well-documented property of generic population models on momentary mood from phone-only passive sensing — not a pipeline bug.
...
Five files, location to be decided (suggest `wellbeing_pipeline/` subdirectory of `Behavioral Emotional Sensing/`):
...
- Project root: `Behavioral Emotional Sensing/`
